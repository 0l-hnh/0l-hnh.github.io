...
|RUN ["커맨드","파라미터1","파라미터2"]|FROM의 베이스 이미지에서 커맨드를 실행|
|ADD <소스><컨테이너 내 경로>|소스(파일, 디렉터리, tar 파일, URL)를 컨테이너 내 경로에 복사|
|ADD ["소스","컨테이너 내 경로"]|소스(파일, 디렉터리, tar 파일, URL)를 컨테이너 내 경로에 복사|
|COPY <소스> <컨테이너 내 경로>|소스(파일, 디렉터리)를 컨테이너 내 경로에 복사|
|COPY ["소스","컨테이너 내 경로"]|소스(파일, 디렉터리)를 컨테이너 내 경로에 복사|
|ENTRYPOINT ["실행 가능한 것", "파라미터"]|컨테이너가 실행하는 파일을 설정|
|ENTRYPOINT <커맨드> <파라미터>|컨테이너가 실행하는 파일을 설정|
|CMD ["실행 바이너리","파라미터"]|컨테이너 기동 시 실행될 커맨드를 지정|
|CMD <커맨드>(shell 형식)|컨테이너 기동 시 실행될 커맨드를 지정|
|CMD ["파라미터1", "파라미터2"]|ENTRYPOINT의 파라미터로 사용됨|
